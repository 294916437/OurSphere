...
@Component
@RocketMQMessageListener(consumerGroup = "oursphere_group_" + MQConstants.TOPIC_COUNT_NOTE_LIKE_2_DB, // Group 组
        ...
        if (CollUtil.isNotEmpty(countList)) {
            ...
            countList.forEach(item -> {
                ...
                transactionTemplate.execute(status -> {
                    try {
                        noteCountPOMapper.insertOrUpdateLikeTotalByNoteId(count, noteId);
                        userCountPOMapper.insertOrUpdateLikeTotalByUserId(count, creatorId);
                        return true;
                    } catch (Exception ex) {
