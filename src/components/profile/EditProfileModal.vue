  ...
  >
    <div class="p-6">
      ...
      <div class="space-y-6">
        ...
        <!-- 用户APP号 -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">用户APP号</label>
          <div class="relative">
            <input
              v-model="form.userId"
              type="text"
              class="w-full h-10 px-3 border border-gray-200 rounded-lg focus:outline-none focus:border-[#ff2442]"
              placeholder="请输入用户APP号"
              maxlength="15"
              @input="updateuserIdCount"
            />
            <span class="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-gray-400">
              {{ userIdCount }}/15
            </span>
...
// 更新oursphere号字数计数
const updateuserIdCount = () => {
  userIdCount.value = form.value.userId.length
}
...
const handleConfirm = () => {
  ...
  if (userId.length < 6 || userId.length > 15) {
    message.show('oursphere号长度应为 6-15 个字符')
    return
  }

  // oursphere号格式验证：必须包含英文字母，且只能包含英文、数字和下划线
  const hasLetter = /[a-zA-Z]/.test(userId)
  const validFormat = /^[a-zA-Z0-9_]+$/.test(userId)

  if (!hasLetter) {
    message.show('oursphere号必须包含英文字母')
    return
  }

  if (!validFormat) {
    message.show('oursphere号只能包含英文字母、数字和下划线')
    return
  }
