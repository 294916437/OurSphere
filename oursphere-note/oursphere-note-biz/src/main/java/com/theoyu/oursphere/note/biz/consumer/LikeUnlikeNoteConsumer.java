...
import com.theoyu.oursphere.note.biz.model.mapper.NoteLikePOMapper;
...
public class LikeUnlikeNoteConsumer implements RocketMQListener<Message> {
    ...
    private void handleLikeNoteTagMessage(String bodyJsonStr) {
        ...
        // 构建 DO 对象
        NoteLikePO noteLikeDO = NoteLikePO.builder()
                .userId(userId)
                .noteId(noteId)
        ...
        // 添加或更新笔记点赞记录
        int count = noteLikePOMapper.insertOrUpdate(noteLikeDO);
    ...
    private void handleUnlikeNoteTagMessage(String bodyJsonStr) {
        ...
        // 取消点赞：记录更新
        int count = noteLikePOMapper.update2UnlikeByUserIdAndNoteId(noteLikeDO);
        if (count == 0) return;
